...
- [ ] 第 3 批 · 进阶（HotUpdateService ✅ Network ✅(core 半 + engine 半均已落地) · ECS 扩展选型定案 bitECS[性能优先，独立包 `@cck/ecs-bitecs` 不进 core]，实现暂缓 · MVVM 数据绑定增强未开始 · tools 包已起：hot-update-manifest ✅）
...
| 工具 | Excel→JSON 配表转换 | tools | 未开始（`config-excel.md` 另开） | — | — |
