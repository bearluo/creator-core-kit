...
- [x] monorepo 骨架 — 根+core+engine、test/typecheck/build/lint 全绿；**demo 消费 core 已在真 cc 3.8.7 验证**（bare `import '@cck/core'` 经 node_modules，预览打出 `[CCK-DEMO]` 全绿）
...
|---|---|---|---|---|---|
| 骨架 | monorepo（pnpm workspace + vitest + lint 依赖约束） | 根 | 已实现（4/4 验收；demo 消费 core 在真 cc 3.8.7 验证） | `docs/design/modules/monorepo-scaffold.md` | — |
...
| 1 地基 | Bootstrap 启动流程（组合根 `boot`/`coreModule` + engine `bootCoreKit`/帧驱动） | core/engine | 已实现（core 20 测试覆盖 100% + engine 11 测试） | `packages/core/docs/modules/bootstrap.md` | — |
