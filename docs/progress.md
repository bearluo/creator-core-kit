...
| 设备分档 | device-tier（档位状态 + 判档编排） | core | **设计定稿，未实现**（engine 半为零。会话内不可变，**只给 `resolveAtStartup()`、不给 `setTier()`** —— 让「运行中切档」在类型层面写不出来；判档另起一个启动步、排在 `dispatch` 后 `shared` 前[`shared` 那步要装按档取的常驻地基皮包，所以判档那一刻只有 base 层 + 网络可用 ⇒ 策略表走 `ConfigTable` 从业务包读**在物理上行不通**]，`LaunchPhase` 加 `tier` 成员[封闭联合，项目自己 splice 的步骤给不了自己准确的 phase]；缓存优先 + 短超时机会主义等待、不重试；**`scoreTier` / `fetchTier` 都由项目注入 —— kit 一行经验值都不送**[默认打分函数要跨平台就必须把三个内存字段挑一个用，正是 #37 禁掉的事]） | `docs/design/device-tiering-overview.md` | hlgit #21 · #23 · #25 · #27 |
