...
| 设备分档 | perf-window（性能采集） | core/engine | **设计定稿，未实现**（原型已 `node demo.ts` 直跑验证，分支 `proto/perf-window`。一个纯类、**无 token 无 KitModule** —— kit 里没有任何东西要 resolve 它；三个接口一个都不做，数据单向：全 push 进来、一份 `PerfReport[]` 出去；直方图定长 1KB、`frame()` 零分配。⭐ **P95 抓不到卡顿**[3/1800 帧 = 0.17%，要 p99.9 才够得着] —— 分位数量稳态帧率，卡顿靠计数 + 累计毫秒，阈值可配。engine 半只有 `drivePerfWindow()` 四行，存在的唯一理由是堵两个**静默**的坑：`ITimer.onFrame` 拿到的是 `dt × timeScale` 且 `paused` 时根本不触发[游戏一暂停数据就断，而暂停可能正是卡出来的]，以及 `game.deltaTime` 是秒而 `frame(dtMs)` 要毫秒） | `docs/design/device-tiering-overview.md` | hlgit #21 · #26 |
