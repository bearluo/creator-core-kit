...
| 3 进阶 | crash-reporting（JS 未捕获异常上报） | core/engine | **JS 半已实现**（core `createCrashFilter`/`crashPayload` 15 测试全绿[去重/种类上限/截首行/载荷不含指纹/getter 抛也不断报]；engine `installCrashReporter` 装 `globalThis.__errorHandler`、Android 以外 no-op，**刻意无单测**——`native.reflection` 是 JNI，按 [[adr-0002]] 禁止进 cc mock，故逻辑全下沉 core；八门全绿[全仓 1151]）；**Java 半 + 真机验证未开始**（gradle flavor 骨架 hlgit #52，两家 SDK 实现与真机 hlgit #50） | `packages/core/docs/modules/crash-reporting.md` | hlgit #42 |
