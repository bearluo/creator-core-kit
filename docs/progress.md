...
| 设备分档 | device-profile（设备画像：内存三件套 / 屏幕 / CPU / 压缩格式支持 / 上次退出原因） | core/engine | **设计定稿，未实现**（`DEVICE_PROFILE: Token<DeviceProfile>` 装一份**启动快照**而不是取数器 —— 业务自己也要读画像，留 `read()` 会让崩溃上报那个「每报一次现取」的 `getContext` 把 JNI 桥打满；且「这是快照不是永恒真理」由类型自己说，不用靠文档记住[CPU 核数在 Android / iOS 都不是可缓存常量]。「取不到」靠 `readFailures` 分三态：平台没这能力 / 本该有却静默失败。内存**刻意不合并成一个数** —— 四平台上那不是同一个量。engine 半按平台注册，另出裸 `readDeviceProfile()` 给周期采样[顺手掐掉 perf-window 反向依赖它]。五项待真机实测） | `docs/design/device-tiering-overview.md` | hlgit #21 · #22 · #37 |
