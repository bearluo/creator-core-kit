...
- [ ] 第 3 批 · 进阶（HotUpdateService ✅ Network ✅(core 半 + engine 半均已落地) · ECS 扩展选型定案 bitECS[性能优先，独立包 `@cck/ecs-bitecs` 不进 core]，实现暂缓 · MVVM 数据绑定增强 ✅(core 响应式原语 + engine 绑定 helper；engine 半真机 gameView 预览验证 PASS) · tools 包：hot-update-manifest ✅ config-excel ✅ compat-stamp ✅）
