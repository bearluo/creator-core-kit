...
- [x] 第 3 批 · 进阶（HotUpdateService ✅ Network ✅(core 半 + engine 半均已落地) · ECS 扩展 ✅(bitECS v0.3.40 接入范例：独立包 `@cck/ecs-bitecs` 不进 core，re-export 全套 + 薄 kit 胶水，6 测试；**demo cc 渲染场景留后续**) · MVVM 数据绑定增强 ✅(core 响应式原语 + engine 绑定 helper；engine 半真机 gameView 预览验证 PASS) · tools 包：hot-update-manifest ✅ config-excel ✅ compat-stamp ✅）
