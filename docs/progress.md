...
| 3 进阶 | crash-reporting（JS 未捕获异常上报） | core/engine | **JS 半已实现**（core `createCrashFilter`/`crashPayload` 15 测试全绿[去重/种类上限/截首行/载荷不含指纹/getter 抛也不断报]；engine `installCrashReporter` 装 `globalThis.__errorHandler`、Android 以外 no-op，**刻意无单测**——`native.reflection` 是 JNI，按 [[adr-0002]] 禁止进 cc mock，故逻辑全下沉 core；八门全绿[全仓 1151]）；**原生骨架已实现**（`channels.json` 驱动 3 个 flavor + `cap-report-logcat` 一份共享实现 + `--channel` + `-keep` + `pnpm check:channels`；三个 flavor 的 `compile*DebugJavaWithJavac` 实测全过、`CckReport.class` 产出确认）；**两家真 SDK 与真机验证未开始**（hlgit #50） | `packages/core/docs/modules/crash-reporting.md` | hlgit #42 |
