...
1. ✅ dispatcher 曾配成 `http://172.25.50.135:8081/cdn/`（filebrowser 的 SPA 路由，任何路径都回
   200 + HTML）。[server-core-kit#1](https://gitlab.huanchanghuyu.com/luohao/server-core-kit/-/issues/1)
...
| 渲染骨架 | camera-rig（kit 常驻相机组）+ resolution（横竖屏锁短边适配） | engine | 已实现（纯决策逻辑 `render-policy.ts` **零 cc**、29 测试全绿[含 `computeCameraCenter` 3 条回归 + `computeSafeAreaInsets` 6 条 + 转屏换轴 3 条]；cc 薄壳 `camera-rig.ts`/`resolution.ts` 按 ADR-0002 决策 3 不进 mock，靠 `tsc -b` 官方真类型 + demo 预览验证；四门全绿[全仓 410]；**apps/demo Game View 预览实测 0 error**：跨 `loadScene` 常驻渲染 / 场景内 `RenderRoot2D` 被常驻相机按 layer 渲染 / `Widget` 满屏锚定 三项 PASS，**转屏与 `claimClear` 两项待真机**；**安全区 2026-09-09 Android 实测通**：`cc.SafeArea` 已经把「Widget 锚定 + 转屏跟随 + 非异形屏退化」做完了，**kit 一行适配代码都不写**，只补它给不了的数 `getSafeAreaInsets()`。模拟器 `cmd overlay enable ...cutout.emulation.hole` 造出 136px 挖孔 → 读到 `上136 下136`（下边那 136 是 `symmetric` 补的，2400 里让掉 272）；`waterfall` 曲面屏读到全 0 —— **这是对的**：`CocosActivity` 的 `SHORT_EDGES` 让窗口在长边上避开挖孔（实测 `mAppBounds=Rect(53,0-1027,2337)`），压根没铺上去。demo 的 manifest 锁死 `portrait`，转屏改到 **web 产物**上验：注一行 `--safe-top:136px` 的 CSS 变量就能造刘海，**`mini-plane` 与 `mini-fish` 的返回键各下移 272 设计单位、分毫不差**；把视口拉成横的（真 resize 事件、零手动调用）可视区 `1080×2400 → 2400×1080`、`Safe` 容器跟着变 `2128×536` —— **安全区自己跟着转屏重算，且用的是换完设计分辨率之后的尺寸**。⚠️ 桌面浏览器上 `symmetric` 认的方向不跟窗口比例走（引擎按 `screenAdapter.orientation` 决定镜像哪根轴），真机不会这样。**顺带补了两道 prefab 闸**（`apps/demo/test/prefab-safe-area.test.ts` / `prefab-integrity.test.ts`）：前者落地当天逮到 **6 张 HUD、19 处贴边却不在安全区里**，最狠的是 `mini-fish` 返回键距顶 5 设计单位 —— 136 的挖孔能把它整个盖住，**「顶部按钮点不到」原来是真的在**；后者保证「用脚本给 prefab 加组件」不会做出「运行时能 instantiate、编辑器一打开就崩」的东西） | `packages/engine/docs/modules/camera-rig.md` | — |
...
| 工具 | source-maps（sourcemap 归档与还原） | tools | **已实现并在真产物上验通**（2026-09-10）。16 测试；`findSourceMaps`/`stashSourceMaps`/`symbolicate` + CLI `cck-manifest stash-maps`/`symbolicate`，解码用 `@jridgewell/trace-mapping`（devDep，tsup 打进 bin）。两份 build-config 改 `sourceMaps: true`，`build.mjs` 在 **Creator 构建之后第一步**把 `.map` 全搬进 `local.json` 的 `mapsDir`（**必须在对外目录之外** —— 实测 CDN 那条分享链 `…/dl/<id>/releases/` 返回 200，整个 CDN 根是公开的），搬完**原地复扫、还剩就抛**。⚠️ 闸只能挡在这里：`.map` 同时走**三条**路出去 —— 热更包 / **APK**（gradle 打包 `data/`）/ **web 目录**（`cpSync(DATA, webDir)`），而最后一条**不经 `deployToCdn`**。**真产物推翻了开票时的两个前提**（app.asar 里读到的 `addChunk` 不是 release 走的那条路）：① **`.map` 名不带 md5，`.js` 才带**（`index.8f281.js` 配 `index.js.map`）—— 原名归档会让各版本互相覆盖、且跟堆栈里的文件名对不上，改成按同目录兄弟 js **补指纹**；② map 里 `sources` **被拼了两遍**（`../file:/…/m/file:/…/m/PlaneVM.ts`），显示时取最后一段。⭐ map 自带 `sourcesContent`，于是还原时**直接打出那一行源码**，不必签出那个 commit（也正是「一份都不能出门」的最直白理由）。端到端实证：真 `index.07346.js:565:128` → `PlaneVM.ts:305:18 planeSolidAt` + 源码原文，标识符对得上。⚠️ `application.<md5>.js` 没有 map（模板生成），那一层的帧还不回去。⚠️ `trace-mapping` 吃 1 基行 / **0 基列**，差一格会静默指到旁边那个函数 —— 测试那份手写 map 的两处刻意不对称就是钉它和 `LEAST_UPPER_BOUND` 的（两处都实测过：改回错写法恰好对应用例变红） | `packages/tools/docs/modules/source-maps.md` | hlgit #42 · #54 · #55 |
| 3 进阶 | crash-reporting（JS 未捕获异常上报） | core/engine | **已实现，两家后台真机跑通**（2026-09-07 · Android 14 / x86_64 模拟器）。core 24 测试[去重/种类上限/截首行/载荷不含指纹/getter 抛也不断报/**堆栈解析** —— `parseJsFrames` 是为 Crashlytics 存在的：它 Android 侧没有「上报自定义堆栈文本」的 API，只能造 `Throwable` + `setStackTrace`，而拆解有分支、有畸形行，属逻辑不能写在测不到的 Java 层]；engine `installCrashReporter` **先 `init` 再装钩子**（Bugly 不 `initCrashReport` 完全不工作，钩子先装上那条异常就白丢），仍**刻意无单测**（[[adr-0002]]）。Java 侧 `com.cck.report.CckReport.{init,report}(String)` 一家一份，flavor 选。**Bugly（qq 包）**：`CRASH TYPE: H5` / `JsError` / 完整 JS 帧 / `APP VER` 是热更版本 / `[Upload] Success: crash` HTTP 200。**Firebase（google 包）**：手写 `firebase.xml` 六条 string 让 `FirebaseApp` 起来了（坐实 ADR-0022 那条唯一没有一手依据的决策），但另有**两个启动前提**，少一个 App 直接打不开 —— `com.crashlytics.RequireBuildId=false` 与 BoM 钉死 33.1.2（[[adr-0023]]）。**顺带修掉一个静默 bug**：模板把 `app/src` 当 main 的递归源根，三份能力实现被编进每个 flavor —— 只有一份实现时它是隐形的，是 ADR-0021 的全 flavor 编译抓出来的；新增第十道门 `check-flavor-classes.mjs` 挡这一类（含 AGP 增量陈旧：报 UP-TO-DATE、全绿、产物里一个 class 都没有）。九门 + 新闸全绿[全仓 1160]。**从后台堆栈回到代码**分两段：「哪一版 / 哪个 commit」**已实跑**（2026-09-10：产物指纹逐包 → `grep releases/` → `releases/<v>/source.json`，出处在构建**开始时**取 —— 构建自己会改 app 戳与 Creator 的 `information.json`，等到归档再问 `git status` 则 `dirty` 恒为 true）；「行号 → 源码行」**已补齐**（2026-09-10，见同表 source-maps 一行）。**三步整条链已在真机上走通**：真崩溃的 `assets/main/index.86397.js:375:19` → `grep releases/` = 1.3.9 → `source.json` 的 commit → `symbolicate` → `Bootstrap.ts:258:13` 加那一行源码原文；引擎帧（`jsb-adapter/…`）原样透出。**上下文也补上了「跑的是哪一版 base」**：`apk`（`__cckBaseEntry`，只有发新包才变）与 `base`（`__cckBaseRunning`，每次 base 热更都变）**两个字段不合并** —— 堆栈的行号对的是后者，只看前者会拿新代码的堆栈去查旧代码。同一个 APK 连更三版实测 `apk` 全程 `6ba8b` 不动而 `base` 走 `6ba8b → 096ca → f2f84 → 6ba8b`（末次退回干净代码，内容相同故同名），埋的 `HOTFIX_TAG` 在 APK 里 `grep -c` = 0、force-stop 全新 PID 仍是新值。⚠️ 那一行 `window.__cckBaseRunning = applicationJs` **必须挂在指针解析那个 IIFE 之后**（四条提前 return 里只有成功那条改了值）。Crashlytics 那条路的前提已补上：它渲染的是重建的 `StackTraceElement`，四个字段里**没有「列」**，所以 `recordException` 之前多一句 `fc.log(stack)` 把原文挂上（Bugly 存的本来就是原文）。**上下文补齐了「这是谁」**（2026-09-10）：`player` 取自地基层的 `AUTH_SESSION`，而主包 `import` 不得 —— 走的是**扩已有的那条唯一接缝**（`js.getClassByName` + 地基入口类的**静态** `playerId()`），零新代码边（`check:graph` 仍是 24 条）。否决了另外两条：主包直接 import 违反铁律；把 token 挪进 `boot/` 让地基反向 import 会开 `foundation → main` 的先例且 token 住进 base。**拉不推**（没有 `setUser()`，忘了调就静默少一块）。真机双向验过：登录**之前**抛的那条 `player="-"` `scene="Boot"`，登录**之后**那条 `player="f2777f66…"` `scene="Lobby"`，与 `[CCK-AUTH] 已认证 player=` 一字不差 —— 顺带说明 getter 确实是**现取**。⚠️ 这一轮**没出新 APK**：改的是 base 与地基两层，热更就够 | `packages/core/docs/modules/crash-reporting.md` | hlgit #42 · #50 |
| 设备分档 | device-profile（设备画像） | core/engine | **已实现并真机跑通**（2026-09-09 · 两台都 `readFailures=[]`：**真 Galaxy SM-S948U / One UI / Android 16 · API 36 · arm64 · SM8850**，加一台 Android 14 x86_64 模拟器作对照）（core 27 测试、`device-profile.ts` 覆盖 **100%**；engine 薄壳按 [[adr-0002]] 无测，API 存在性由 `tsc -b` 用 `@cocos/creator-types` 真类型保证）。`DEVICE_PROFILE: Token<DeviceProfile>` 装的是**启动快照不是取数器** —— 业务自己也要读画像，留 `read()` 会让崩溃上报那个「每报一次现取」的 `getContext` 把 JNI 桥打满；另出裸 `readDeviceProfile()` 给周期采样，顺手掐掉 perf-window 反向依赖它。内存**刻意拆三个字段不合并**（四平台上那不是同一个量）；「取不到」靠 `readFailures` 分三态（平台没这能力 / 本该有却静默失败），且**刻意不写进类型**。⭐ **原生桥一次调用返回一整个 JSON**（`()Ljava/lang/String;`）：十几个字段只走一次 JNI、完全绕开「`J`(long) 返回可不可用」那个未验证项、而且解析成了**纯逻辑**；类不存在时同步抛 → 当作「没这能力」不记失败。`REASON_*` 映射放 core 不放 Java（Java 那半就没有分支可测），**17 个常量值取自 `javap -constants` 读 `android.jar`(android-35)，不是凭记忆** —— 总纲那条待验证项就此关掉；未知码翻 `unknown-<码>` 而不并进 `other`（后者是个有确切含义的取值）。**真机实测**：JNI 桥整条链到位（内存三件套/lowRam/核数/dpi/brand/model/abis）；⭐ **#22 待实测①「`/sys` 在 app 进程里读不读得到」= 读得到，而且真机上值是真的**（`主频=3628800kHz` = 3.63 GHz）—— 不是模拟器、不是 shell 域，是真 One UI 上 app 域的实读。细节：同目录 `cpuinfo_cur_freq` 是 root only，而 `cpuinfo_max_freq` 世界可读，**系统收紧的是「当前频率」不是「最大频率」**，我们要的恰好是后者；`Build.SOC_MODEL` 过了 API 31 版本门（`soc=ranchu`）；**`REASON_*` 映射端到端验证** —— 第二次启动读到 `上次退出=userRequested`（`am force-stop` → `REASON_USER_REQUESTED`(10)），**那张从 `javap` 读 `android.jar` 抠出来的常量表是对的**；首次启动「没有上次」被正确判成缺席而非读失败。⚠️ 模拟器上 `cpuinfo_max_freq` 读出来是 `2`（虚拟 CPU 的垃圾值），真机 `3628800` —— **「读得到」不等于「值可信」**，打分函数要自己防，而且**这一点在模拟器上永远发现不了**（那台「读到了」，只是读到个假的）。另一层意外收获：那台是 **API 36**，比工程 `compileSdk 35` 还新，**十几项 API 一个都没因版本新而失效**。**仍未验**：非三星厂商（Redmi / realme / vivo / OPPO）的收紧情况 —— **印度低端市场主力不是三星**，且这次那台是**旗舰**（11GB / 8 核 / 3.6GHz），低端机行为仍空白；**web 已接**（2026-09-10）：`cpuCores`/`densityDpi`/`deviceTotalMemoryBytes` 三项，浏览器实测 `核数=14 dpi=160`、`readFailures=[]`。⚠️ **`navigator.deviceMemory` 只在 HTTPS 下有** —— 实测 http 页面 `isSecureContext=false` 时它恒 `undefined`（而同页 `hardwareConcurrency`/`devicePixelRatio` 照给），H5 挂 http 上就永远没有内存这一项，缺席**不记 readFailures**（浏览器按规矩不给，不是读坏了）；且它被量化并**封顶 8**，16G 的机器也报 8。`densityDpi` 用 `dpr × 160` 而不是 CSS 的 96 —— 那个字段的语义由 Android `DisplayMetrics.densityDpi` 定，用 96 会让同一台手机在原生与 H5 上差 1.67 倍。`performance.memory.jsHeapSizeLimit` **刻意不用**（http 下也读得到、~2G，但那是 V8 堆上限，跟 Android 的 Java 堆上限 192–256M 差一个数量级）。**小游戏仍未接**。⚠️ 真机跑法两个坑：`appABIs` 默认只有 `x86_64`（装真机前要临时改 arm64 重编）；**云端设备连不到内网**，demo 启动必挂在 `dispatch` —— 所以画像日志刻意打在 `boot()` 之后 `launch()` 之前，启动成不成功都照出。 | `packages/engine/docs/modules/device-profile.md` | hlgit #21 · #22 · #37 |
| 设备分档 | device-tier（档位状态 + 判档编排）+ `UIVariant.tier` | core | **已实现并真机跑通**（2026-09-09：`启动阶段 → tier` 确实夹在 `hotupdate` 与 `shared` 之间；首次启动 `档位 → high（来源 local）`、第二次 `（来源 cache）` —— 缓存往返成功，也就顺带验证了那两个**下划线 slot 名**真的存下来了。27 测试、`device-tier.ts` 语句/函数/行 **100%**、分支 90.3%；`app` 侧另 4 条；UI 侧 2 条。**engine 半为零**）。会话内不可变，**只给 `resolveAtStartup()`、不给 `setTier()`** —— 让「运行中切档」在类型层面写不出来。判档优先级：玩家自选 > 服务器 > 缓存 > 本地打分 > default（**玩家自选就不再问服务器**，启动还快一步）。**不阻塞启动**：缓存优先 + 短超时赛跑、不重试；「服务器明说没有」与「网络挂了」启动路径行为相同，差别只进独立的 `serverOutcome` 字段（一个是覆盖率问题一个是可用性问题，塞进 `source` 就分不开了）；**没赶上的那趟迟到回来仍写缓存给下次用** —— 而它跑在启动之后，写之前必须再判一次宿主还在不在。`scoreTier`/`fetchTier` **全由项目注入，kit 一行经验值都不送**。启动步排在 `hotupdate` 后 `shared` 前（后半段是硬约束：`shared` 要装按档取的常驻地基皮包；前半段是取舍：热更可能 halt 去重启，排后面不白判）；**`AppConfig` 刻意不加字段**，`tier` 步只问容器里注册了没有，没注册整步跳过。`LaunchPhase` 加 `tier` 成员，封闭联合**当场在编译期点名了 demo 的 `LaunchOverlay`** —— 这正是否决「让项目自己 splice tierStep」的硬理由（外部扩不了联合，插的步骤只能借一个不相干的 phase）。⚠️ 三个静默坑：**`SaveManager` 的 slot 名会被静默剔字符**（`cck.tier`→`ccktier`，改用下划线）；**`setVariant` 的早退判据原是列举式**，加 `tier` 不改那行会让 `setUIVariant({tier})` 彻底 no-op（实测：换回列举式两条测试立刻红，报 `expected default to be high`），已改成逐字段比；**`LaunchOverlay` 的 `ORDER` 数组不受类型保护**（`LaunchPhase[]` 少一个成员完全合法，漏了进度条那段不动）。另修：`scoreTier` 是项目代码且同步调用，它抛异常原本会掀翻整条启动序列 —— 分档是可选功能不该有这个能力，已在源头吞掉并降级成 default。 | `packages/core/docs/modules/device-tier.md` | hlgit #21 · #23 · #24 · #25 |
| 设备分档 | perf-window（性能采集） | core/engine | **已实现**（core 18 测试、`perf-window.ts` 覆盖 **100%**；engine 3 测试；八门全绿[全仓 1181]）。一个纯类、**无 token 无 KitModule** —— kit 里没有任何东西要 resolve 它；三个接口一个都不做[采样源天然 push / 上报端点是项目协议 / 内存读数一 pull 就得认识平台]，数据单向：全 push 进来、一份 `PerfReport[]` 出去，node 里逐帧可复现；直方图定长 1KB、`frame()` 零分配。⭐ **P95 抓不到卡顿**[30 秒 60fps 夹三次卡顿 = 0.17% 的帧，p50/p95/p99 全是 17ms] —— 分位数量的是稳态帧率，卡顿靠计数 + 累计毫秒，阈值可配；**已固化成回归测试**，别「优化」掉 stalls。engine 半只有 `drivePerfWindow()` 四行，存在的唯一理由是堵两个**静默**的坑：`ITimer.onFrame` 拿到的是 `dt × timeScale` 且 `paused` 时根本不触发[游戏一暂停数据就断，而暂停可能正是卡出来的]，以及 `game.deltaTime` 是**秒**而 `frame(dtMs)` 要**毫秒**[差 1000 倍，两边都是「一个正数」类型系统一个字不说，直方图会整体挤进 0 号桶而看着还挺正常]——所以那四行**也必须有测**。**促成主干时改掉原型一处静默 bug**：封段判据原本是 `_frames === 0`，于是「零帧但有 `span`」的段（加载）既不产报告、状态又不清，那份数据会在下一次 `mark()` 时漂到**下一个 tag** 名下 —— 把判据换回原型版本，18 条里恰好那一条红。 | `packages/core/docs/modules/perf-window.md` | hlgit #21 · #26 |
