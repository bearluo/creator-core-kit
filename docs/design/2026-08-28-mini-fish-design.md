...
- **玩什么**：横屏捕鱼。**上下对坐四门炮**（下左是玩家，另外三个角是 AI 陪打），点屏幕开炮，子弹飞到命中点
...
16 个文件平铺在一个目录里，读的人看不出**哪些是那四道接缝**（设计里最重要的一件事）。
按「这是什么角色」分四层，不按技术类型分：

```
modules/mini-fish/
├─ Fish.scene            场景（只有一个 FishRoot + FishGame）
├─ FishGame.ts           View —— 本模块**唯一**带 cc 的文件
├─ FishVM.ts             装配：把接缝、炮台、system 流水线串起来
├─ events.ts             VM → View 的事件（query 看不出来的那部分）
├─ art/                  图集 textures.png/.plist · 背景 seabed.jpg
│  ├─ water.effect       水面后处理：折射 + 焦散 + 气泡（见「水下氛围」）
│  ├─ hit-flash.effect   受击闪白
│  └─ noise.png          pnpm gen:noise 烘的三层噪声（生成物）
├─ content/              这一款的**内容**：改它不动结构
│  ├─ fish-kinds.ts      11 种鱼：帧数 / 半径 / 倍率 / 是不是炸弹
│  └─ paths.ts           场地尺寸 + 8 条贝塞尔泳道
├─ ecs/                  走 ECS 的那三样（鱼 / 子弹 / 网）
│  ├─ components.ts      SoA 组件
│  ├─ feedSystem.ts · pathSystem.ts · bulletSystem.ts · netSystem.ts · despawnSystem.ts
│  └─ reconcile.ts       enterQuery/exitQuery → 建哪些节点、拆哪些节点（零 cc，见 §11）
└─ seams/                **四道接缝**：今天单机，明天换服务端
   ├─ economy.ts         ① 裁决 FishArbiter + ② 钱包 FishWallet（接口 + 本地实现）
   ├─ wallet-storage.ts  ② 的单机落盘实现
   ├─ agent.ts           ③ 炮台指令源 CannonAgent（玩家 / AI 同一个接口）
   └─ feeder.ts          ④ 鱼池投喂源 FishFeeder
```

测试路径**镜像**这棵树（`test/modules/mini-fish/ecs/bulletSystem.test.ts` …），
`check:vm-tests` 就是按镜像路径找的。搬文件时 `.meta` 必须跟着走 —— `Fish.scene` 是按
`compressUuid(FishGame.ts 的 uuid)` 引用组件的，uuid 换了场景当场找不到脚本。

### 四门炮：上下对坐

```
        ┌─────────────────────────────┐
   AI2  │ ▼ (-480, 360)   (480, 360) ▼│  AI3      ← 上半场，炮口朝下
        │                             │
        │          鱼在这里游          │
        │                             │
 玩家   │ ▲ (-480,-360)  (480,-360) ▲ │  AI1      ← 下半场，炮口朝上
        └─────────────────────────────┘
```

一排四个的话中间两门互相挡视线，上半场也没人打、鱼白游。

⚠️ `|y| = 360` 不是随手取的：场地是 **cover 缩放**的，比 16:9 更宽的屏会把上下裁掉一截，
可见半高 ≈ `960 / 宽高比`（21:9 只剩 412，炮台半高 48）。**故意不在 View 里做钳制** ——
那会让画出来的炮口跟 VM 里子弹的出膛点对不上，超宽屏下子弹就凭空从旁边冒出来。

### 水下氛围：一层后处理 + 一次闪白

**水波纹是后处理，不是给海底贴图拧 UV** —— 拧贴图只能让海底自己晃、鱼纹丝不动，看着像
「水在背景里」。要的是「隔着水面往下看」，所以整个水下画面先进 RenderTexture：

```
WATER 层（bit 1）──→ WaterCamera（只看这层，priority 100）──→ RenderTexture
   海底 · 鱼                                                     │
                                                                 ↓ 采样
UI_2D 层 ─────────────────────────→ kit 那台 ui 相机（priority 200）
   ├─ WaterView   全屏 quad，走 art/water.effect   ← 折射 + 焦散 + 气泡都在这一个 pass
   ├─ Bullets     子弹（见下：直线穿过位移场就不再是直线）
   ├─ Effects     渔网 · 死亡动画
   ├─ Cannons     四门炮
   └─ Hud         金币 / 炮等级 / 返回
```

进 RT 的只有**海底和鱼**；**子弹、渔网、炮台、HUD 一点都不扭** —— 玩家射出去的东西
跟着水晃会让人以为自己瞄歪了。子弹还有个更硬的理由：**折射是个空间上变化的位移场**
（每个像素按它所在位置的噪声被拧走十几像素，噪声本身还在滚），一条直线穿过去就不再是
直线。鱼大且慢，只表现为「微微变形」；子弹细长又每帧走很远，整条轨迹会被拧成抖动的
波浪，还跟不扭的炮口衔接不上 —— 一开始把子弹沉进水里，现场就是这样。
⚠️ `node.layer` **不继承**，每帧新生的鱼节点都要逐个置层。

一个 pass 干三件事，所以整个水面只占一个 draw call：

| | 怎么做的 | 为什么 |
|---|---|---|
| 折射 | 两层**反向**滚动的噪声，取**不同通道**当 x/y 偏移 | 同向滚 = 整幅画在平移，看不出水在动；同通道 = 只会沿对角线抖 |
| 焦散 | 两层等值线**相交处**才亮（`pow(1-abs(n1.g-n2.r), 5)`） | 相交带天然是网状的，正是阳光透过水面那种游动光网 |
| 气泡 | 一列一个，每像素只查**相邻三列**；画**环**不画实心圆 | 开销与气泡数无关；实心的看着像光球，气泡是薄壳、亮的是边 |

噪声图 `art/noise.png` 由 `pnpm gen:noise` 烘：64×64（**POT 才能开 REPEAT** —— 波纹靠 UV
一直往外滚，clamp 会把整幅图糊成边缘那一行像素），R/G/B 三通道装 4×4 / 8×8 / 16×16 三层频率，
一次采样拿三层。它是脚本产物但**不配 `--check` 闸**：源就是那个脚本、没有仓外美术要同步
（判据同 `mini-hop` 的 `level.ts`）。

**受击闪白**（`art/hit-flash.effect`）：挨了一网没死的鱼白闪 0.12 秒（`netSystem` 发 `hurt` 事件）。
非要一个 effect，是因为 `Sprite.color` 是**乘算**的 —— 只能把鱼调暗，白色乘上去等于没变。
全场受击的鱼共用**同一个** Material（强度是常量、不随时间衰减），闪完换回内置材质；
代价是这 0.12 秒里鱼群的合批被切开几段。

⚠️ **采 RT 必须翻 y，而且这活得自己干。** GL 系后端往 framebuffer 写的第 0 行是屏幕**底**边，
采样时 `uv.y=0` 拿的就是第 0 行 —— 整幅水下画面上下颠倒。引擎备了这条路，但只走内置材质：
`Sprite._updateBuiltinMaterial` 发现贴图是 `RenderTexture` 时，复制一份打了 `SAMPLE_FROM_RT`
define 的材质，`builtin-sprite.effect` 里那行 `CC_HANDLE_RT_SAMPLE_FLIP(uv0)` 才生效。
**用 `customMaterial` 就够不着那段**，得自己在 shader 里调一次宏。宏自带后端判断
（`cc_cameraPos.w` 编码了屏幕空间 y 的符号，GL 系 > 1.0），所以一行同时管住 WebGL 与
Metal / Vulkan。只翻取 RT 颜色那一次：噪声 / 焦散 / 气泡都是屏幕空间效果，照旧吃没翻过的 `uv0`。
...
7. ~~**横屏大厅放不下 10 个入口**~~ —— **已解决**（2026-08-28，本设计之外）：入口列表改成按横竖屏
   排的网格 + `ScrollView`，竖屏上下滚（2 列）、横屏左右滚（7 行），装得下就不滚。
   横屏下第 10 个入口现在落在屏内 (535, 291)，e2e 里直接点进捕鱼。见
   [`lobby-modular-framework-overview`](lobby-modular-framework-overview.md#9-已知行为与坑)。
8. **AI 档位固定 3 级**（`AI_LEVEL`）。第三刀压量时再说要不要让它们变档。
9. **`enterQuery` / `exitQuery` 是两个数组，读不出交错顺序** —— 真踩到过（2026-08-28），
   现场是「屏幕正中央停着十来个不动的子弹」。同一个 eid **一帧内可以同时**出现在两边：

   | 交错 | 怎么来的 | 照着数组直接做会怎样 |
   |---|---|---|
   | enter → exit（**出生即死**） | 子弹在炮口出生的那一帧就命中了正巧游过炮口的鱼 | 先跑 exit（表里还没有，空转）再跑 enter → 建出一个**实体已不存在**的节点：它不在 query 里，位置永不刷新，就停在 `(0,0)`，也永不再 exit → 永久孤儿 |
   | exit → enter（**eid 被回收重发**） | `removeEntity` 把 eid 还进池子，同帧 `addEntity` 又取出来 | 新租客的节点被 exit 那步拆掉，或 enter 覆盖了表里的旧节点、旧节点从此没人销毁 |

   判据只有一条：**这一帧结束时它还在不在 `query(world)` 里**。收在 `ecs/reconcile.ts`
   （零 `cc`、6 例单测）：`exited` 且不在场 → 拆；`entered` 且在场 → 建（建之前先拆同 eid 的旧节点）；
   `entered` 但已不在场 → 这一帧当它没来过。**鱼也有同一个 bug**，只是鱼不会出生即死
   （`feedSystem` 生出来 `t=0`，`despawnSystem` 要 `t>=1`），所以一直没现形。

   ⚠️ 这条是**所有 ECS→节点桥的通病**，不是捕鱼特有的。下一个用 `@cck/ecs-bitecs` 建节点的
   模块请直接复用这个判据。
