...
- **测试结果 / 覆盖率**：`vitest` 全绿（core 共 **54 测试**：DI 20 + Logger 13 + EventBus 18 + 骨架 3）；`tsc -b`、`eslint` 干净。`eventbus.ts` 覆盖 **Stmts/Branch/Funcs/Lines 全 100%**；core All files 100/98.19/100/100（剩余 branch 缺口来自 DI/logger 早前已接受的防御分支）。
- **commit / PR**：待提交。
