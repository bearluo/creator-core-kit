...
# ADR-0017：AOT 层走固定名指针，从此可热更（重启生效）

**取代 [[adr-0016]] 的决策 4 与它「AOT 层只能整包更新」那条后果。** 0016 的其余决策全部有效。

## 背景

ADR-0016 判定 base manifest 的 asset 表恒空，理由是「下发了也没人读」：引用 AOT 的是产物根上的
...
用户对边界的要求也正好落在这里：**AOT 改动重启就该生效，只有引擎指纹变才必须发 APK。**

## 决策

1. **AOT 入口运行时解析**。`main.js` 读固定名 `src/cck-aot.json` 的 `application` 字段拿入口名；
...
3. **base manifest 装 AOT 整条链**：`application.<md5>.js`（经 `ManifestOptions.files` 收，它在产物
   根、子目录遍历够不着）+ `src/cck-aot.json` + `src/settings.<md5>.json` + `src/chunks/**` +
...
4. **`assets/resources` 进 base，app 戳因此随 AOT 一起更新**。这是必须的：热更换掉
...
6. **「APK 换没换」的判据换源**：从运行时 `settings.bundleVers.main` 换成 `main.js` 烘进来的
   `window.__cckAotEntry`（`packagedAotEntry()` → `aotStamp()`）。不换会**自噬**：`settings` 现在
   随热更走，每成功更新一次 AOT 判据就翻一次 → 缓存被当成「上一版 APK 攒的」整个删掉 → 下轮重下
   再删，死循环。`main.js` 属 L0，烘在里面的名字是唯一不可伪造的 APK 身份。
7. **出包侧硬闸**：`project.manifest` 里没有 AOT 入口或没有指针 → 构建失败。少了它，「AOT 热更
...
- 决策 6 换源之后，判据比以前**更敏感**（入口名随 `settings` md5 走，而 `settings` 含全部
  `bundleVers`）：新出的 APK 只改了个日志文案也会让老缓存整个作废。换来的是不会出现「新 AOT 配旧模块」。

## 验证

- tools 单测：`isEngineBound` 逐条边界（含「前缀相近不误伤」）、`files` 收散文件、`contentHashed`
  下 base 的进出清单、`main.js` 永不进表。engine 单测：`aotStamp` 换源后的取值与休眠、`packagedAotEntry`。
