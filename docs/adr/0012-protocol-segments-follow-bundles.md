...
**Cocos Creator 3.8.7 把所有 npm 依赖统一打进 `src/chunks/bundle.js`**，那是随主包走的 AOT 层——改了要整包更新、用户重装（ADR-0009 的分层判据里，main/res 就是"需重启"的那一层）。谁 import 它不影响这个结果：实测让 `assets/modules/mini-clicker/` 下的脚本 import 契约包，代码照样落在 AOT chunk 里。
...
- **基础段**（握手 / 心跳 / 错误 / 分配器，cmd 1–999）留在 AOT，走 npm `exports`。它本来就不该常变，改它要整包更新是可以接受的代价。
...
- protobufjs 运行时留在 AOT 共享。实测模块 bundle 的产物里它出现 **0 次**——运行时不会被复制进每个 bundle，涨的只有协议本身。
...
codec 拿的不再是一张造好就不动的表，而是可增量的注册表：AOT 启动时装基础段并注册进 `PB_SCHEMA` token，模块 bundle 加载时 `add` 自己那段、注销函数交 `BundleScope.add()` 托管。全程 `INetwork` 与 codec 实例不换、连接不断。
...
- **模块段产物里内联了它 import 的框架 proto**（pbjs 的行为）。这是特性不是浪费：bundle 能热更、AOT 不能，模块协议绝不能依赖 AOT 里那份可能已经过期的副本。
