...
**端口被占不会让你去猜**：起之前先查，全被 ssh 占且一个不落 → 「已经有一条在转发，直接用」
并**退 0**（重复跑、终端起了再点菜单，都不报错）；被 ssh 占但**没占全** → 指出少了哪个端口
（多半是隧道起来后这份文件又加了地址，必须停掉重开，否则「隧道开着偏偏某个服务连不上」）；
被别的进程占 → 报出进程名与 pid 和两条出路。
