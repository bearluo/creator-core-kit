...
1. **主包不得 `import` 地基的任何值** —— 那段代码会被判给主包（优先级最高者赢）→ 地基进 base
   ...
   主包要问地基「现在是什么状态」（如崩溃上报要的玩家 ID，登录态住在地基）就**扩这条缝**：
   给地基入口类加**静态**方法、主包按类名现问。静态而非实例：`boot()` 那个实例活在闭包里、
   要一路 plumbing 才拿得出来，而且地基热更换了类之后存下来的是**上一份**。
   ⚠️ **别反过来把 token 挪进 `boot/` 让地基 import** —— 那会开 `foundation → main` 的边
   （合法但拓扑里至今没有），token 还跟着住进 base。也**别用 `setUser()` 式的推**：
   忘了调就静默少一块，见 [[crash-reporting]] 的 getter 现取。
