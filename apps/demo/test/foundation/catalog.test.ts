...
describe('skinBundle', () => {
  ...
  it('一个跟随者一个包 —— 这是「不下载没用到的模块的皮」的全部机制', () => {
    ...
    const names = new Set(['foundation', 'lobby', 'mail'].map((o) => skinBundle(o)(v('default'))));
...
describe('registerCatalogUIs · 换皮登记', () => {
  ...
  it('登记了 skinned 的模块从自己的皮包取脸', () => {
    ...
    expect(resolveUIDef(def, v('default'))).toEqual({ bundle: 'skin-default-mail', prefab: 'Mail' });
  ...
  it('每个 skinned 模块的皮包名都由自己的 id 决定 —— 不共包', () => {
    ...
    for (const e of skinned) {
      expect(resolveUIDef(getUIDef(e.id)!, v('default')).bundle).toBe(`skin-default-${e.id}`);
    }
  });
});
