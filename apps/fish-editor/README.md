...
2. **鱼阵页**：编一阵里有几**群**鱼、每群走哪条路、什么鱼、几条、速度，以及**队形**。
   队形直接在画布上拖：选中的那群按「路径起点 + 起点切线」画成半透明的鱼 + 一圈虚线判定半径，
   拖哪条改哪条，落点会被推开到不重叠。右栏那排「重排」（一列 / 横排 / 雁阵 / 一簇 + 间距）是**起手**，
   微调靠拖。走带能播能拖能单步 —— **跑的是游戏本体的 `FishVM`**，不是仿的，所以看到的摆动
   就是游戏里的摆动。
   路径 / 鱼种的下拉带**模糊筛选**：敲 `crh` 就能中 `cross-rl-high`。
...
| 要什么 | 从哪来 |
|---|---|
| 编的是什么（草稿 / 拖点 / 分段 / 队形 / 导出 / 落盘） | `src/EditorVM.ts`（零 `cc`，node 直跑） |
