...
日志行（SpineLabDriver.runFidelity）：
  [SpineFidelity] page=P clip=C frame=F lerp=L fps=R rect=x0,y0,x1,y1 dx=D window=WxH renderer=ui|mesh
...
LINE = re.compile(
    ...
    r"rect=(-?\d+),(-?\d+),(-?\d+),(-?\d+) dx=(-?\d+) window=(\d+)x(\d+)(?: renderer=(\w+))?"
)
...
def main() -> int:
    ...
    for page, shot in enumerate(shots):
        ...
        for (row_page, frame), match in sorted(rows.items()):
            ...
            renderer = match.group(12) or "ui"
            variant = (renderer, "half" if frame % 1 else "whole", match.group(4))
            worst[variant] = max(worst.get(variant, 0.0), over48)
            print(f"page={page} renderer={renderer} clip={match.group(2)} frame={frame} lerp={match.group(4)} mean={mean:.2f} >16={over16:.2%} >48={over48:.2%}")
            if out:
    ...
    for (renderer, kind, lerp), value in sorted(worst.items()):
        print(f"worst >48 renderer={renderer} frame={kind} lerp={lerp}: {value:.2%}")
    return 0
