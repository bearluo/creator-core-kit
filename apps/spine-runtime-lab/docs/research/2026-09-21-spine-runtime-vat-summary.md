...
状态：已定稿（调研快照）
摘要：2026-09-20～21 官方 Runtime 源码分析、VAT PoC 与中低端真机对照的汇总报告。
何时读：想了解 VAT 立项依据与早期实测时。
依赖：[官方 Runtime 结论](2026-09-21-spine-runtime-findings.md)
...
该模拟器的 95.2% 进程 CPU 包含 SwiftShader 软件光栅化，不能作为真机 CPU 收益。它证明的是 Native JSB 的自定义实例属性、离线数据加载和 GLES instancing 链路已跑通；早期 `spine-vat-1` 的 ARM64 真机性能数据不能直接冒充 M3 真机回归。播放器现状见 [Spine VAT Player](../design/spine-vat-player.md)（M3 当时的复现方法与模拟器实测只在 git 历史里）。
...
- [Spine VAT Compiler](../design/spine-vat-compiler.md)（现状）
- [Spine VAT Player](../design/spine-vat-player.md)（现状）
