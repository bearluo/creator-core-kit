...
状态：已定稿（调研快照）
摘要：Cocos Creator 3.8.7 官方 Spine Runtime 的运行模型、缓存模式与合批结论。
何时读：评估官方 Spine 能否满足同屏数量、或解释 VAT 为什么有收益时。
依赖：无

完整源码链路见 [spine-runtime-memory-flow.md](2026-09-21-spine-runtime-memory-flow.md)，VAT 是否上线见 [spine-vat-decision.md](2026-09-21-spine-vat-decision.md)。
