...
- 烘焙与真机测试流程见工程 README。
- 单测：`test/bake/SpineVatFixedLayout.test.ts`（槽位 / 退化 / 剪裁区 / 规则）、`test/bake/SpineVatCompilerV2.test.ts`（manifest 与静态区）、`test/extensions/spine-vat-importer/runtime/SpineVatRenderResources.test.ts`（3D 网格索引与材质宏）。
