...
#### CPU / 内存 / 帧 / 渲染对比（2026-09-24 实测）

同一台机、同一套场景，release 包（整理目录后打的包，运行时代码与上表相同）。每阶段 20 秒：+6 秒起 10 秒读 `/proc`（App 进程 CPU 时间 ÷ 全机 CPU 时间），之后 `dumpsys meminfo`，帧与渲染取该阶段最后一条 `[SpinePerf]`。原始数据：`artifacts/cliptest/device/report-r1/`（没入库）。

**CPU**（App 占整机 8 核 % ／ top 口径，100% = 占满 1 核）：

| 实例 | 2D 插值 | 2D 阶跃 | 3D 插值 | 官方 SHARED_CACHE |
|---|---|---|---|---|
| 30 | 10.9% ／ 87% | 10.7% ／ 86% | 10.3% ／ 82% | 12.4% ／ 99% |
| 150 | 9.3% ／ 74% | 9.1% ／ 73% | 8.9% ／ 71% | 16.8% ／ 134% |
| 600 | 11.1% ／ 89% | 11.0% ／ 88% | 10.6% ／ 85% | 18.1% ／ 145% |

同期全机总 CPU 占用 22.9% ~ 28.8%。

**帧**（fps ／ 帧间隔 p50 ／ p95，ms）：

| 实例 | 2D 插值 | 2D 阶跃 | 3D 插值 | 官方 SHARED_CACHE |
|---|---|---|---|---|
| 30 | 60.2 ／ 16.6 ／ 17.2 | 60.2 ／ 16.6 ／ 17.2 | 60.1 ／ 16.5 ／ 17.4 | 60.1 ／ 16.6 ／ 17.0 |
| 150 | 60.2 ／ 16.6 ／ 16.9 | 60.2 ／ 16.6 ／ 16.8 | 60.2 ／ 16.6 ／ 16.9 | 60.2 ／ 16.6 ／ 16.8 |
| 600 | 60.1 ／ 16.6 ／ 16.8 | 60.2 ／ 16.6 ／ 16.8 | 60.2 ／ 16.6 ／ 16.8 | 24.4 ／ 42.5 ／ 48.0 |

**渲染**（DC ／ 三角形）：

| 实例 | 2D 插值 | 2D 阶跃 | 3D 插值 | 官方 SHARED_CACHE |
|---|---|---|---|---|
| 30 | 3 ／ 2.96 万 | 3 ／ 2.96 万 | 6 ／ 2.96 万 | 107 ／ 2.96 万 |
| 150 | 8 ／ 14.8 万 | 8 ／ 14.8 万 | 6 ／ 14.8 万 | 530 ／ 14.8 万 |
| 600 | 23 ／ 59.3 万 | 23 ／ 59.3 万 | 6 ／ 59.3 万 | 1956 ／ 59.3 万 |

**内存**（进程 PSS ／ Native Heap ／ Graphics，MB；末行是引擎统计的 GFX 纹理 ／ 缓冲）：

| 实例 | 2D 插值 | 2D 阶跃 | 3D 插值 | 官方 SHARED_CACHE |
|---|---|---|---|---|
| 30 | 562 ／ 324 ／ 154 | 565 ／ 325 ／ 154 | 563 ／ 326 ／ 154 | 495 ／ 319 ／ 112 |
| 150 | 539 ／ 320 ／ 154 | 550 ／ 324 ／ 154 | 552 ／ 320 ／ 154 | 550 ／ 324 ／ 159 |
| 600 | 557 ／ 330 ／ 146 | 569 ／ 347 ／ 137 | 599 ／ 366 ／ 132 | 626 ／ 346 ／ 152 |
| GFX 纹理 ／ 缓冲 | 18.7 ／ 57.8 | 18.7 ／ 57.8 | 18.7 ／ 57.9 | 16.0 ／ 57.8 |

- 内存这张表不能当各模式的真实占用看：12 个阶段在同一进程里按表中顺序跑，两份资源（官方 Spine 与 VAT）从保真测试起一直常驻（A/B 开始前的基线 PSS 564.6 MB），Native Heap 随阶段推进从 324 涨到 366，越靠后读数越偏高。能直接对比的只有 GFX 纹理：VAT 比官方多 2.7 MB，即 VAT 数据贴图。要得到各模式单独的内存，需要每种模式冷启动、只加载自己的资源各测一次（驱动目前不支持）。
