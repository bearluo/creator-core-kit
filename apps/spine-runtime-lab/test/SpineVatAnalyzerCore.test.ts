...
} from '../assets/scripts/SpineVatAnalyzerCore';
import type { SpineVatFrameProbe } from '../assets/scripts/SpineVatTypes';
...
describe('SpineVatAnalyzerCore', () => {
  it('识别 Spine 4.2 clipping、mesh、动画时间线和 atlas', () => {
    const result = analyzeSpineJson(
      readJson('assets/resources/tuan42/tuan.json'),
      readText('assets/resources/tuan42/tuan.atlas'),
    );

    expect(result.spineVersion).toBe('4.2.43');
    expect(result.runtimeFamily).toBe('4.2');
    expect(result.atlasPages.map((page) => page.name)).toEqual(['tuan.png']);
    ...
    expect(result.features.timelines.drawOrder).toBe(true);
  });

  it('将 Spine 3.8 标成独立 worker，并保留约束统计', () => {
    const result = analyzeSpineJson(
      readJson('assets/resources/spineboy-pro.json'),
      readText('assets/resources/spineboy-pro.atlas'),
    );

    expect(result.runtimeFamily).toBe('3.8');
    expect(result.features.constraints.ik).toBe(7);
    expect(result.features.constraints.transform).toBe(7);
